...
$user_id = "1";
	...
	$sql = "insert into user_activities(user_id,plant_id,part_id,activity_id,evaluation_status,findings,shift,reponsible_person,actual_closed_date,created_date,status) values (?,?,?,?,?,?,?,?,?,now(),?)";
	try {
		$dbCon = getConnection();
		...
		for ($x = 0; $x<$size; $x++) {
			...
			$stmt->bindParam(4, $activityval[$x]);
			$stmt->bindParam(5, $evaluation);
			$stmt->bindParam(6, $findings);
			$stmt->bindParam(7, $shift);
			$stmt->bindParam(8, $reponsible);
			$stmt->bindParam(9, $datepicker);
...
function getConnection() {
    try {
        $db_username = "root";
        $db_password = "123456";
        $conn = new PDO('mysql:host=localhost;dbname=db_heart_new', $db_username, $db_password);
